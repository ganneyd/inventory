...
import 'package:inventory_v1/domain/usecases/order/create_part_order.dart';
...
void main() {
  ...
  late CreatePartOrderUsecase sut;
  ...
  setUp(() {
    ...
    sut = CreatePartOrderUsecase(mockPartOrderRepo);
  });

  group('.call()', () {
    test('should return right', () async {
      when(() => mockPartOrderRepo.createPartOrder(orderEntity))
          .thenAnswer((invocation) async => const Right<Failure, void>(null));

      var results =
          await sut.call(CreatePartOrderParams(orderEntity: orderEntity));

      verify(() => mockPartOrderRepo.createPartOrder(orderEntity)).called(1);
      expect(results, const Right<Failure, void>(null));
    });

    test('should return left', () async {
      when(() => mockPartOrderRepo.createPartOrder(orderEntity)).thenAnswer(
